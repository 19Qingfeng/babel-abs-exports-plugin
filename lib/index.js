    ...
    return {
        visitor: {
            ...
            VariableDeclarator(nodePath) {
                var _a, _b;
                const { node } = nodePath;
                if (node.__$done || generatorList.includes(node)) {
                    return;
                }
                const { id, init } = node;
                ...
                // 转化后的节点
                const { replaceNode, originalNode, transformNodes } = (0, require_1.generateRequireExp)(original, transform);
                // 标记生成的节点
                ...
                if (originalNode) {
                    // replaceNode.forEach(node => Object.defineProperty(node, 'dsadf', {
                    //   value: true
                    // }))
                }
                replaceNode.forEach(node => generatorList.push(node));
                nodePath.replaceWithMultiple(replaceNode);
                nodePath.skip();
            }
        },
